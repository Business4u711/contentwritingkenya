...
export function CompanyStory() {
  return (
    <section id="story" className="py-32 relative bg-brand-surface border-y border-brand-border">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center mb-24">
          <div>
            ...
            <div className="space-y-6 text-brand-text-muted font-light leading-relaxed mb-8">
              <p>
            ...
            <div className="glass-panel p-6 border-l-4 border-l-brand-gold">
              <p className="text-lg font-heading font-semibold text-brand-text mb-2 uppercase tracking-tight">Our Mission</p>
          ...
          <div className="relative h-full min-h-[400px]">
            ...
            <div className="absolute inset-0 glass-panel border border-brand-border p-8 flex flex-col justify-end group overflow-hidden">
               ...
               <svg viewBox="0 0 100 100" className="absolute top-10 left-10 w-[150%] h-[150%] text-brand-accent/20 stroke-current stroke-[0.2]">
                 <path d="M0,80 Q25,40 50,60 T100,20" fill="none" className="drop-shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
                 <path d="M0,90 Q30,60 60,70 T100,40" fill="none" className="text-brand-primary/20 stroke-current drop-shadow-[0_0_10px_rgba(197,160,89,0.5)]" />
               </svg>
               
               <div className="relative z-20">
                 <div className="inline-block px-3 py-1 bg-brand-text flex items-center justify-center text-brand-bg text-[10px] uppercase tracking-widest font-bold mb-4">
                   Nairobi HQ
                 </div>
                 <h3 className="text-3xl font-heading font-black uppercase text-brand-text mb-2">Global Operations</h3>
                 <p className="text-brand-text-muted text-xs font-bold uppercase tracking-[0.2em]">Executing for ambitious brands worldwide.</p>
               </div>
            </div>
          </div>
        </div>
        ...
        {/* Core Values */}
        <div>
          <div className="flex items-center gap-2 mb-10 text-center justify-center uppercase tracking-[0.3em] text-[10px] font-bold text-brand-text-muted">
            ...
            {values.map((val, idx) => {
              ...
              return (
                <motion.div 
                  ...
                  className="glass-panel p-6 flex flex-col items-center justify-center text-center gap-4 hover:bg-brand-surface-light transition-colors cursor-default border-t border-brand-border border-l border-brand-border"
                >
                  <Icon className="w-6 h-6 text-brand-primary" />
                  <span className="text-brand-text-muted font-bold uppercase tracking-widest text-[10px]">{val.text}</span>
                </motion.div>
              );
            })}
          </div>
