      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 mb-32 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {industries.map((ind, i) => (
          <Link to={ind.path} key={i}>
            <motion.div
              className="glass-panel p-8 border-white/10 group hover:border-brand-gold/30 hover:bg-brand-gold/5 transition-colors h-full"
              whileHover={{ y: -5 }}
            >
              <div className="text-white/40 group-hover:text-brand-gold transition-colors mb-6">
                {ind.icon}
              </div>
              <h3 className="text-xl font-bold text-white mb-2">{ind.title}</h3>
              <div className="inline-block px-3 py-1 bg-brand-gold/10 text-brand-gold text-[10px] font-bold tracking-widest uppercase mb-8">
                {ind.metrics}
              </div>
              
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-white/30 group-hover:text-white transition-colors">
