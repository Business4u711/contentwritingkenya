    ...
    <div className="pt-32 pb-16">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10 w-full mb-24">
        <div className="flex items-center gap-2 mb-6 uppercase tracking-[0.3em] text-[10px] font-bold text-brand-accent">
          <Link to="/services" className="text-brand-text-faint hover:text-brand-text pb-1">Services</Link>
          <ChevronRight className="w-3 h-3 text-brand-text-faint" />
          <span className="pb-1">Technical SEO</span>
        </div>
        <h1 className="text-5xl md:text-7xl font-heading font-black uppercase tracking-tighter text-brand-text mb-6 leading-tight">
          Algorithmic <br/>
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-accent to-white">Perfection.</span>
        </h1>
        <p className="text-brand-text-muted font-light text-lg max-w-2xl leading-relaxed mb-10">
        ...
        <button className="px-8 py-4 bg-brand-text text-brand-bg font-bold uppercase text-[10px] tracking-widest hover:bg-brand-accent hover:text-brand-text transition-colors">
          Audit My Tech Stack
        </button>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-24">
        <div className="grid lg:grid-cols-2 gap-16">
          <div>
             <h2 className="text-3xl font-heading font-black uppercase tracking-tight text-brand-text mb-6">Core Capabilities</h2>
             <p className="text-brand-text-muted mb-12">Search engines operate on strict render budgets. If your JavaScript framework or server configuration causes latency, you lose rankings automatically. We fix the foundation.</p>
             ...
             <div className="space-y-6">
                {[
                  ...
                  <div key={i} className="flex gap-4 p-6 bg-white/[0.02] border border-brand-border rounded-lg hover:border-brand-accent/20 transition-colors">
                    <div className="text-brand-accent shrink-0">{item.icon}</div>
                    <div>
                      <h4 className="text-brand-text font-bold mb-2">{item.title}</h4>
                      <p className="text-brand-text-faint text-sm">{item.desc}</p>
                    </div>
                  </div>
                ))}
             </div>
          ...
          <div className="glass-panel p-8 md:p-12 border-brand-border relative overflow-hidden bg-gradient-to-br from-[#0a0a0f] to-[#050505]">
            <div className="absolute top-0 right-0 w-64 h-64 bg-brand-accent/10 blur-[100px] pointer-events-none" />
            <h3 className="text-2xl font-heading font-bold text-brand-text mb-8 border-b border-brand-border pb-4">The Technical Playbook</h3>
